...
<main>
  ...
  <section class="section container"><div class="section-head"><div><p class="eyebrow">La collection</p><h2>Choisir une montre qui vous ressemble.</h2></div><p>Finitions, cadrans et matériaux se répondent pour composer une présence juste, au quotidien comme aux grandes occasions.</p></div><div class="product-grid"><?php foreach ($products as $slug => $product): ?><article class="product-card"><a href="<?= e(url('/product.php?watch=' . rawurlencode($slug))) ?>"><img src="<?= e(url('/' . $product['image'])) ?>" alt="<?= e($product['name']) ?>"></a><div class="product-card__copy"><p class="eyebrow"><?= e($product['sku']) ?></p><h3><?= e($product['name']) ?></h3><div class="product-card__meta"><span><?= e($product['short'] ?? $product['bracelet']) ?></span><strong><?= money($product['price']) ?></strong></div><?php $ratingProduct = $product; $ratingModifier = 'product-rating--card'; require APP_ROOT . '/templates/product-rating.php'; ?><a class="text-link" href="<?= e(url('/product.php?watch=' . rawurlencode($slug))) ?>">Voir la montre</a></div></article><?php endforeach; ?></div></section>
</main>
