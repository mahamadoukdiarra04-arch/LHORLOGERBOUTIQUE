...
<?php else: ?>
  ...
  <section class="admin-panel" style="margin-top:15px"><p class="admin-kicker">Classement détaillé</p><h2>Produits par rentabilité réalisée.</h2><div class="admin-table-wrap mobile-card-table-wrap"><table class="admin-table mobile-card-table"><thead><tr><th>Produit</th><th>Ventes web</th><th>CA net réalisé</th><th>CMV réalisé</th><th>Charges directes</th><th>Contribution</th><th>Suivi Meta</th><th>CAC marketing</th><th>Stock</th></tr></thead><tbody><?php foreach ($rows as $index => $row): ?><tr class="mobile-card-row"><td data-label="Produit"><strong><?= str_pad((string) ($index + 1), 2, '0', STR_PAD_LEFT) ?> · <?= e($row['name']) ?></strong><small><?= e($row['slug']) ?></small></td><td data-label="Ventes web"><?= $row['web_units'] ?> unité(s)</td><td data-label="CA net réalisé"><strong><?= $row['is_realized'] ? money($row['net_revenue_fcfa']) : 'Aucune écriture' ?></strong></td><td data-label="CMV réalisé"><?= $row['is_realized'] ? money($row['cogs_fcfa']) : '—' ?></td><td data-label="Charges directes"><?= $row['is_realized'] ? money($row['direct_expense_fcfa']) : '—' ?></td><td data-label="Contribution"><strong><?= $row['is_realized'] ? money($row['contribution_fcfa']) : '—' ?></strong></td><td data-label="Suivi Meta"><?= $row['meta_tracking_fcfa'] > 0 ? money($row['meta_tracking_fcfa']) : 'À renseigner' ?></td><td data-label="CAC marketing"><?= $row['marketing_cac_fcfa'] !== null ? money($row['marketing_cac_fcfa']) : ($row['web_units'] > 0 ? 'Coût Meta à renseigner' : 'Aucune vente web') ?></td><td data-label="Stock"><span class="status <?= (int) $row['stock'] <= 6 ? '' : 'delivered' ?>"><?= (int) $row['stock'] ?> unité(s)</span></td></tr><?php endforeach; ?><?php if ($rows === []): ?><tr class="mobile-card-empty"><td colspan="9">Aucun produit actif.</td></tr><?php endif; ?></tbody></table></div></section>
<?php endif; ?>
