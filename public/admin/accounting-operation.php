...
<?php if (isset($operationError)): ?><section class="admin-panel"><p class="flash flash-error"><?= e($operationError) ?></p></section><?php else: $operation = $detail['operation']; $displayLabel = accounting_journal_display_label($operation); ?>
<section class="operation-hero"><div><span><?= e($operation['group_type']) ?> · <?= e($operation['status']) ?></span><strong><?= e($displayLabel) ?></strong><small><?= e($operation['reference']) ?> · groupe <?= e($operation['group_reference']) ?></small></div><b class="<?= $operation['nature'] === 'disbursement' ? 'amount-negative' : '' ?>"><?= $operation['nature'] === 'disbursement' ? '−' : ($operation['nature'] === 'receipt' ? '+' : '↔') ?><?= money($operation['amount_fcfa']) ?></b></section>
