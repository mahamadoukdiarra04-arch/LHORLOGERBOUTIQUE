...
?>
<header class="admin-page-head accounting-head"><div><p class="admin-kicker">Comptabilité · Journal</p><h1>Toutes les traces, dans l’ordre.</h1><p>Les écritures confirmées ne sont jamais modifiées ; une correction passe par une contrepassation visible.</p></div><div class="accounting-head-actions"><a class="admin-button admin-button--secondary" href="<?= e(url('/admin/accounting-journal.php?group_type=delivery&status=confirmed')) ?>">Ventes livrées & encaissées</a><a class="admin-button" href="<?= e(url('/admin/accounting-action.php')) ?>">+ Nouvelle action</a></div></header>
...
<section class="admin-panel accounting-filter-panel"><form class="accounting-filter" method="get">
  ...
  <label>Origine<select name="group_type"><option value="">Toutes</option><?php foreach (accounting_journal_group_labels() as $groupType => $groupLabel): ?><option value="<?= e($groupType) ?>" <?= $filters['group_type'] === $groupType ? 'selected' : '' ?>><?= e($groupLabel) ?></option><?php endforeach; ?></select></label>
...
<section class="admin-panel accounting-table-panel"><div class="admin-table-wrap"><table class="admin-table accounting-table"><thead><tr><th>Date</th><th>Écriture</th><th>Type</th><th>Compte</th><th>Catégorie</th><th>Montant</th><th></th></tr></thead><tbody><?php foreach ($journal['rows'] as $operation): $displayLabel = accounting_journal_display_label($operation); ?><tr><td><?= e(date('d/m/Y H:i', strtotime($operation['effective_at']))) ?><small><?= e($operation['status']) ?></small></td><td><strong><?= e($displayLabel) ?></strong><?php if ($operation['counterparty']): ?><small><?= e($operation['counterparty']) ?></small><?php endif; ?><small><?= e($operation['reference']) ?> · <?= e($operation['group_reference']) ?><?= $operation['order_ref'] ? ' · ' . e($operation['order_ref']) : '' ?></small></td><td><span class="status <?= e($operation['nature']) ?>"><?= e($operation['nature'] === 'receipt' ? 'Encaissement' : ($operation['nature'] === 'disbursement' ? 'Décaissement' : 'Transfert')) ?></span></td><td><?= e($operation['account_name']) ?><?php if ($operation['destination_account_name']): ?><small>→ <?= e($operation['destination_account_name']) ?></small><?php endif; ?></td><td><?= e($operation['category_name'] ?? 'Hors résultat') ?></td><td class="<?= $operation['nature'] === 'disbursement' ? 'amount-negative' : '' ?>"><strong><?= $operation['nature'] === 'disbursement' ? '−' : ($operation['nature'] === 'receipt' ? '+' : '↔') ?><?= money($operation['amount_fcfa']) ?></strong></td><td><a class="text-link" href="<?= e(url('/admin/accounting-operation.php?operation=' . (int) $operation['id'])) ?>">Détail</a></td></tr><?php endforeach; ?><?php if ($journal['rows'] === []): ?><tr><td colspan="7" class="admin-table-empty">Aucune écriture ne correspond à ces filtres.</td></tr><?php endif; ?></tbody></table></div>
<div class="journal-cards"><?php foreach ($journal['rows'] as $operation): $displayLabel = accounting_journal_display_label($operation); ?><a class="journal-card" href="<?= e(url('/admin/accounting-operation.php?operation=' . (int) $operation['id'])) ?>"><div><span><?= e(date('d/m/Y H:i', strtotime($operation['effective_at']))) ?> · <?= e($operation['status']) ?></span><strong><?= e($displayLabel) ?></strong><?php if ($operation['counterparty']): ?><small><?= e($operation['counterparty']) ?></small><?php endif; ?><small><?= e($operation['account_name']) ?> · <?= e($operation['group_reference']) ?></small></div><b class="<?= $operation['nature'] === 'disbursement' ? 'amount-negative' : '' ?>"><?= $operation['nature'] === 'disbursement' ? '−' : ($operation['nature'] === 'receipt' ? '+' : '↔') ?><?= money($operation['amount_fcfa']) ?></b></a><?php endforeach; ?></div>
</section>
